    ...
    return new Promise((resolve, reject) => {
      ...
      let req = https.request(options, (res) => {
        ...
        res.on('end', () => {
          ...
          // deleteBill returns empty body and status code 204 No Content
          if (res.statusCode === 204) {
            resolve({ body: {}, req: res.req, res: res });
            return;
          }
          if (res.statusCode < 200 || res.statusCode > 299) {
            const err = new Error(`Non-OK HTTP Status '${res.statusCode}' receieved from Regalii API`)
          ...
          try {
            ...
            reject(new Error(`Non JSON API response from Regalii received: ${body}`))
            return
          }
          resolve({ body: parsed, req: res.req, res: res })
        });
      });
